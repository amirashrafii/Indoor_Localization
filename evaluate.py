...
#taken from Mikaela Angelina Uy's main code
...
DATABASE_FILE= "/home/aa9sz/Driver_in_the_loop/Amir/Indoor Localization/Link Lab Data/Model/withF&C/Hololens_tesset_evaluation_database_1.pickle"
QUERY_FILE= "/home/aa9sz/Driver_in_the_loop/Amir/Indoor Localization/Link Lab Data/Model/withF&C/Hololens_testset_evaluation_query_1.pickle"

# DATABASE_FILE= 'D:/Link Lab Dataset/training_queries_baseline.pickle'
# QUERY_FILE= 'D:/Link Lab Dataset/test_queries_baseline.pickle'

#LOG_DIR = 'transfer/log/PNVLAD-trained/baseline-6pos12neg/'
LOG_DIR = 'log/'
...
def get_recall(sess, ops, m, n):
    ...
    print("len of queries_output: ", len(queries_output))
    ...
    for i in range(len(queries_output)):
        try:
            true_neighbors= QUERY_SETS[n][i][m]    #Same as ground truth to check the extracted distances from the whole database to be compared with the true neighbors
            #print("true neighbors:", set(true_neighbors))
        except:
            print("keyerror")
        if(len(true_neighbors)==0):
            continue
        num_evaluated+=1
        #print(len(queries_output[i].shape))
        #print(queries_output[i].shape)
        if len(queries_output[i].shape) != 1:

            print("find 1D array and skipped")
            break
