...
def main():
    CSV_file = "centroids_test"
    CSV_counter = "PC_locations_6m"
    submap_num = 1
    folder_num = 1
    ...
    for i in range(len(sampl)):
        ...
        os.makedirs("E:/Link Lab Data/with floor and cieling/6X6/Testset/" + str(folder_num) + "/submaps_6m")
        with open("E:/Link Lab Data/with floor and cieling/6X6/Testset/"+ str(folder_num)+"/"+CSV_counter + '.csv', 'w', newline='') as csvfile:
            ...
            for y in np.arange(start_position['y'], -0.436-0.9, -6): #0.9 is the margin for the points
                start_position['y'] = y
                start_position['x'] = x_value
                for x in np.arange(start_position['x'], 30.176+0.9, 6):
                    start_position['x'] = x
                    ...
                    o3d.io.write_point_cloud("E:/Link Lab Data/with floor and cieling/6X6/Testset/"+ str(folder_num)+"/submaps_6m"+"/"+str(submap_num)+".pcd", point_cloud_crop)

                    CSVwriter = csv.writer(csvfile, delimiter=',',
                                           quotechar='|', quoting=csv.QUOTE_MINIMAL)
                    CSVwriter.writerow([str(submap_num)] + [start_position['x']] + [start_position['y']] + [1.7])

                    with open("E:/Link Lab Data/with floor and cieling/6X6/Testset/"+CSV_file+'.csv', 'a', newline='') as csvfile_cetroids:
